    ...
    return <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="container p-4">
                <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                    <div className="p-3 border rounded-lg bg-[var(--background)] md:col-span-8">
                        <Form action={store()} method="POST" className="space-y-4 inert:opacity-50 inert:pointer-events-none">
                            {({errors}) => (
                                ...
                                <Input type="text" className="bg-[var(--color-input)]"  id="title" name="title"/>
                                {errors.title && <InputError message={errors.title} />}
                                <Label htmlFor="content">Blog Content</Label>
                                <Textarea className="w-full h-64 p-2 border rounded-md bg-[var(--color-input)]" id="content" name="content" placeholder="Write your blog content here..."></Textarea>
                                {errors.content && <InputError message={errors.content} />}
                                <Label htmlFor="featured_image">Featured Image</Label>
                                <Input type="file" className="bg-[var(--color-input)]" id="featured_image" name="featured_image"/>
                                {errors.featured_image && <InputError message={errors.featured_image} />}
                                <Label htmlFor="status">Status</Label>
                                <NativeSelect className="bg-[var(--color-input)]" name="status">
                    ...
                    <div className="p-3 border rounded-lg bg-[var(--background)] md:col-span-4">
                    </div>
                </div>
            </div>
    </AppLayout>;
